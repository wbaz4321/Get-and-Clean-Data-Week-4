...
valid_names<-make.names(names=ini_names,unique=T,allow=T)

test_df<-cbind(test_sub,test_x,test_y)
train_df <- cbind(train_sub,train_x,train_y)

names(test_df) <- c("subject", valid_names, "activity")
test_df <- mutate(test_df, dataset = "test")

names(train_df) <- c("subject", valid_names, "activity")
train_df <- mutate(train_df, dataset = "train")

## combine the two data set together train_df (7352x563) test_df (2947x563)
...
data_mean<-sapply(df[,c(2:562)],mean, na.rm=TRUE)
data_std<-sapply(df[,c(2:562)],sd, na.rm=TRUE)

df_group<-group_by(df,subject,activity)

## compute the average value for each group, take use of the valid_names variable
...
## alternatively, we can use "summarise_at()" to do this in a more convenient eay
## tidy_df<-summarise_at(df_group,1:562,mean)
